...
def export_dataset():
    ...
    session_log = os.path.join(
        project_root, "backend", "logs", "session_summaries.jsonl"
    )
    enriched_log = os.path.join(
        project_root, "backend", "logs", "enriched_segments.jsonl"
    )
    emb_log = os.path.join(
        project_root, "backend", "logs", "conversation_embeddings.jsonl"
    )
