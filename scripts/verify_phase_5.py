...
def verify_phase_5():
    ...
    mock_session_summary = {"duration_sec": 300}
    ...
    insight_obj = composer.compose_insight_object(
        session_id, mock_pattern_data, mock_prediction_result, mock_session_summary
    )
    ...
    if not any(
        "decision was made, but no clear next action" in ins for ins in insights
    ):
    ...
    if len(markers) != 3:
        print(
            f"[FAIL] Expected 3 markers (Objection + Dip + Decision), got {len(markers)}"
        )
        sys.exit(1)

    types = [m["type"] for m in markers]
    if (
        "objection" not in types
        or "decision" not in types
        or "sentiment_dip" not in types
    ):
