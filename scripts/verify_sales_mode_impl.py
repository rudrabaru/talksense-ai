...
def test_sales_mode():
    ...
    if detected_objections != expected_objections:
        print(
            f"FAILED: Objections mismatch. Expected {expected_objections}, got {detected_objections}"
        )
        sys.exit(1)
    ...
    # 2b. Buying Signals
    if (
        not res["buying_signals"]
        or "sounds good" not in res["buying_signals"][0]["text"]
    ):
    ...
    if res["summary"]["sentiment_trend"] != "improving":
        print(
            f"FAILED: Trend mismatch. Expected 'improving', got {res['summary']['sentiment_trend']}"
        )
        sys.exit(1)
    ...
    if not expected_actions.issubset(detected_actions):
        print(
            f"FAILED: Actions mismatch. Expected at least {expected_actions}, got {detected_actions}"
        )
        sys.exit(1)
