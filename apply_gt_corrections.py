...
def fix_sales_meeting():
    ...
    with open(
        os.path.join(sample_dir, "ground_truth.json"), "r", encoding="utf-8"
    ) as f:
        gt = json.load(f)
    ...
    gt["description"] = (
        "54s sales call. Sales rep (A) presents product to customer (B). Human-reviewed speaker labels."
    )
...
def fix_sales_bad():
    ...
    gt["description"] = (
        "42s poorly conducted sales call. 2 speakers (rep A, customer B). Pyannote failed to separate — human-reviewed labels."
    )

    backup_and_save(sample_dir, gt)
    print(
        f"  sales_bad: Fixed {len(customer_indices)} segments (indices {customer_indices})"
    )
...
def fix_sales_ambiguous():
    ...
    gt["description"] = (
        "53s sales call with ambiguous buying signals. Sales rep (B) qualifies customer (A). Human-reviewed."
    )

    backup_and_save(sample_dir, gt)
    print("  sales_ambiguous: Fixed 2 segments (indices [9, 11])")
    return 2
...
def fix_meeting_messy():
    ...
    gt["description"] = (
        "39s meeting with overlapping speech. 3 speakers (chair A, participant B, participant C). Human-reviewed."
    )

    backup_and_save(sample_dir, gt)
    print("  meeting_messy: Fixed 6 segments")
    return 6
...
def fix_sales_good():
    ...
    gt["description"] = (
        "78s sales call with positive engagement. Sales rep (A) and customer (B). Human-reviewed."
    )
    backup_and_save(sample_dir, gt)
...
def fix_meeting_clear():
    ...
    gt["description"] = (
        "48s clear meeting with 3 participants. Distinct voices. Human-reviewed."
    )
    backup_and_save(sample_dir, gt)
    print("  meeting_clear: Reviewed, no changes needed")
    return 0
...
def fix_business_english_long():
    ...
    gt["description"] = (
        "205s ESL business meeting. Tony (chair), Carrie (marketing), Jason. Pyannote groups into 2 voice clusters. Human-reviewed."
    )
    backup_and_save(sample_dir, gt)
    print("  business_english_long: Reviewed, description updated")
    return 0
...
def main():
    ...
    print("  Samples now marked as human_reviewed: 7")
    print(
        "  Remaining unreviewed: meeting_short (already human GT), business_meeting, pitch_competition_long"
    )
    print(f"{'=' * 70}")
