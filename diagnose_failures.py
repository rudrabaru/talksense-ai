...
def main():
    ...
    for cat, name in FAILING_SAMPLES:
        ...
        for i, pred in enumerate(predicted):
            ...
            print(
                f"  {i:3d} {pred.start:7.2f} {pred.end:7.2f} {dur:5.1f} {pred.speaker:>10} {mapped:>6} {gt_match:>4} {match:>5} {text}"
            )
        ...
        for i in range(1, len(predicted) - 1):
            ...
            if p == n and c != p:
                ...
                print(
                    f"    [{i}] {p}→{c}→{p} dur={dur:.1f}s words={wc} text='{predicted[i].text[:40]}'"
                )
        ...
        for i in range(len(predicted) - 1):
            if predicted[i].text and predicted[i].text.strip().endswith("?"):
                ...
                if mp == mn:
                    ...
                    gt_at_next = "?"
                    for g in gt_segs:
                        ov = min(predicted[i + 1].end, g.end) - max(
                            predicted[i + 1].start, g.start
                        )
                        if ov > 0.1:
                    ...
                    gt_at_curr = "?"
                    for g in gt_segs:
                        ov = min(predicted[i].end, g.end) - max(
                            predicted[i].start, g.start
                        )
                        if ov > 0.1:
                    ...
                    if gt_at_curr != gt_at_next:
                        print(
                            f"    [{i}→{i + 1}] pred={mp}→{mn} GT={gt_at_curr}→{gt_at_next} Q='{predicted[i].text[-40:]}'"
                        )
