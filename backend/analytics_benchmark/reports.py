...
def generate_reports(results: Dict[str, Any], output_dir: str):
    ...
    with open(md_path, "w", encoding="utf-8") as f:
        ...
        for module, mets in overall.items():
            f.write(
                f"| {module.capitalize()} | {mets.get('precision', 0):.3f} | {mets.get('recall', 0):.3f} | {mets.get('f1', 0):.3f} |\n"
            )
        f.write("\n")
        ...
        fp_counter = Counter(
            [
                e.get("predicted")
                for e in all_errors
                if e.get("type") == "False Positive"
            ]
        )
        fn_counter = Counter(
            [e.get("expected") for e in all_errors if e.get("type") == "False Negative"]
        )
        ...
        for dataset_id, dataset_res in results.get("datasets", {}).items():
            ...
            for module, mets in dataset_res.items():
                ...
                f.write(
                    f"- **F1:** {mets.get('f1', 0)} (P: {mets.get('precision', 0)}, R: {mets.get('recall', 0)})\n"
                )
                f.write(
                    f"- **TP:** {mets.get('true_positives', 0)}, **FP:** {mets.get('false_positives', 0)}, **FN:** {mets.get('false_negatives', 0)}\n"
                )
                ...
                if matches:
                    ...
                    for m in matches:
                        f.write(
                            f"  - Predicted: *{m.get('predicted')}* | Expected: *{m.get('expected')}*\n"
                        )
                ...
                if errors:
                    ...
                    for err in errors:
                        if err.get("type") == "False Positive":
                            f.write(
                                f"  - 🔴 **FP:** Predicted *{err.get('predicted')}* (Rule: {err.get('rule', 'N/A')})\n"
                            )
