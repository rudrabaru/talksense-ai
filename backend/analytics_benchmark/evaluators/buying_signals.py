...
from services.context_analyzer import assess_sales_signals, BUDGET_ALIGNMENT_KEYWORDS, BUYING_SIGNAL_KEYWORDS
...
def evaluate(dataset: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for s in segments:
        for c in s.get("clauses", []):
            if not c.get("is_negated") and not c.get("is_conditional") and not c.get("is_abandoned") and any(
                kw in c.get("text", "").lower() for kw in ALL_BUYING_KEYWORDS
            ):
