...
from services.context_analyzer import detect_objections


def evaluate(dataset: Dict[str, Any]) -> Dict[str, Any]:
    segments = dataset.get("segments", [])
    expected_texts = dataset.get("expected_objections", [])

    # Run the real extractor
    predicted_objs = detect_objections(segments)

    # predict_objs returns list of dicts: {"text": ..., "keyword": ...}
    predicted_texts = [obj.get("text", "") for obj in predicted_objs]
    ...
    for i, err in enumerate(results.get("errors", [])):
        if err["type"] == "False Positive":
            ...
            for obj in predicted_objs:
                if obj.get("text", "") == pred_text:
                    keyword = obj.get("keyword", "OBJECTION_KEYWORDS")
                    break

            results["errors"][i]["segment_idx"] = source_idx
            results["errors"][i]["speaker"] = speaker
            results["errors"][i]["rule"] = keyword
