...
from db.models import (
    Client as DBClient,
    ClientSnapshot as DBClientSnapshot,
...
BRIEFING_PROMPT = """You are an CRM relationship intelligence assistant for TalkSense AI.

You are given the meeting history for client "{client_name}" in the "{client_industry}" industry.
Your goal is to write a concise, professional relationship briefing summary (2-3 sentences max) that a sales representative can read in 10 seconds before starting their next meeting with this client.

The summary should highlight:
1. The client's main concerns or interests.
2. Sentiment trend and any unresolved objections.
3. Crucial context from recent sessions (e.g. key focus areas, pricing discussions, or integration timelines).

Here is the history of the last {num_sessions} meetings:
{meeting_history_text}

Write only the 2-3 sentence briefing summary. Do not include any greeting, intro, markdown formatting, or prefix. Be direct and actionable.
"""
...
async def update_client_memory(db: AsyncSession, client_id: uuid.UUID) -> None:
    ...
    aggregate common objections, generate a relationship summary using Gemini (or fallback),
    and persist a new ClientSnapshot row.
    """
    ...
    try:
        ...
        if not client:
            logger.error("client_memory — client %s: not found in database", str(client_id)[:8])
            return

        # 2. Fetch all completed sessions for this client ordered chronologically (ascending)
        ...
        if meetings_count == 0:
            logger.info("client_memory — client %s: no completed sessions found", str(client_id)[:8])
            return
            ...
            try:
                ...
                for i, s in enumerate(last_three):
                    ...
                    analysis_res = await db.execute(
                        select(DBAnalysisResult).where(DBAnalysisResult.session_id == s.id)
                    )
                    analysis = analysis_res.scalar_one_or_none()
                    summary_text = analysis.summary if analysis else "No summary available."
                    ...
                    history_parts.append(
                        ...
                        f"  Date: {s.started_at.isoformat() if s.started_at else 'Unknown'}\n"
                        f"  Mode: {s.mode}\n"
                ...
                logger.info("client_memory — client %s: generated LLM summary", str(client_id)[:8])
            except Exception as llm_err:
                logger.error("client_memory — client %s: Gemini call failed, falling back: %s", str(client_id)[:8], llm_err)

        if not summary:
            # Smart fallback template builder
            fallback_parts = []
            if client.industry:
                fallback_parts.append(f"{client.name} is a client in the {client.industry} industry.")
            else:
                fallback_parts.append(f"Client {client.name} has completed {meetings_count} meeting(s) to date.")
            
            if sentiment_trend == "improving":
                fallback_parts.append("Their engagement is improving, indicating a positive outlook.")
            elif sentiment_trend == "declining":
                fallback_parts.append("Their engagement trend is declining, suggesting potential concerns or unresolved issues.")
            else:
                fallback_parts.append("Their engagement trend is stable.")
                
            if common_objections:
                fallback_parts.append(f"Key areas of concern or discussion have focused on: {', '.join(common_objections).lower()}.")
                
            summary = " ".join(fallback_parts)
            logger.info("client_memory — client %s: generated fallback summary", str(client_id)[:8])
        ...
        logger.info(
            "client_memory — client %s: memory update successfully staged [meetings=%d, trend=%s]",
            str(client_id)[:8], meetings_count, sentiment_trend
        )

    except Exception as exc:
        logger.error("client_memory — client %s: failed to update memory: %s", str(client_id)[:8], exc, exc_info=True)
