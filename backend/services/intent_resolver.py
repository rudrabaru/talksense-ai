...
_ROADMAP_SUBJECTS = {"we will", "we'll", "we are going to", "the team will",
                     "our team will", "the product will"}
_ROADMAP_NOUNS = {
    "feature", "features", "update", "updates", "version", "release",
    "product", "platform", "app", "application", "module", "integration",
    "functionality", "capability", "capabilities", "improvement", "roadmap",
}
_ROADMAP_TIME_HORIZON = {
    "next month", "next quarter", "next year", "this quarter", "this year",
    "later this year", "q1", "q2", "q3", "q4", "by end of year",
    "within the year", "upcoming", "soon",
}

# ── INFORMATION signals ──────────────────────────────────────────────────────
_INFO_SUBJECTS = {"the system", "the tool", "the platform", "it ", "that ",
...
def resolve_clause_intent(text: str, flags: dict = None) -> dict:
    ...
    has_roadmap_subject = any(t.startswith(s) or t.startswith(" " + s)
                              for s in _ROADMAP_SUBJECTS)
    ...
    if has_roadmap_subject and has_product_noun and has_time_horizon:
        return {
            ...
            "reason": (
                f"Plural/team subject with product noun and time horizon detected. "
                f"Treated as product roadmap announcement, not a personal task."
            ),
            "confidence": 0.90,
        }
