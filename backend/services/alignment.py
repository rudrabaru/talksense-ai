...
class WordAligner:
    ...
    """
    @staticmethod
    def align_words(whisper_segments: list, speaker_intervals: list[dict]) -> list[dict]:
        """
        ...
        for seg in whisper_segments:
            words = getattr(seg, "words", []) if not isinstance(seg, dict) else seg.get("words", [])
            if not words:
                ...
                seg_start = getattr(seg, "start_time", 0.0) if not isinstance(seg, dict) else seg.get("start_time", 0.0)
                seg_end = getattr(seg, "end_time", 0.0) if not isinstance(seg, dict) else seg.get("end_time", 0.0)
                ...
                for w in words:
                    ...
                    w_start = getattr(w, "start", 0.0) if not isinstance(w, dict) else w.get("start", 0.0)
                    w_end = getattr(w, "end", 0.0) if not isinstance(w, dict) else w.get("end", 0.0)
                    w_prob = getattr(w, "probability", 0.0) if not isinstance(w, dict) else w.get("probability", 0.0)
        ...
        for w in all_words:
            ...
            if best_speaker == "Unknown":
                ...
                for interval in intervals:
                    dist = min(abs(w_start - interval["end"]), abs(w_end - interval["start"]))
    ...
    def process(aligned_words: list[dict]) -> list[dict]:
        ...
        for w in aligned_words:
            ...
            if current_segment is None:
                ...
                # Speaker switch
                # Build text
                current_segment["text"] = "".join(x["word"] for x in current_segment["words"]).strip()
