...
DEFER_TERMS = ["later this year", "few months", "next quarter", "sometime later"]
...
def compose_executive_summary(
    ...
    quality = assess_meeting_quality(
        decisions, uncontrolled_issues, explicit_no_blockers
    )
    ...
    if quality == "good":
        summary = (
            f"The discussion focused on {topic}, with key decisions locked in and clear next steps assigned. "  # noqa: E501
        ...
        if explicit_no_blockers:
            summary = summary.replace(
                "Minor issues were identified", "Execution details were addressed"
            )

    # Case B (ATTENTION): Decisions + Uncontrolled Issues
    elif quality == "needs_attention" and len(decisions) > 0:
        summary = f"The discussion addressed {topic} and resulted in decisions, but some issues remain unresolved and require follow-up."  # noqa: E501

    # Case C (POOR): No Decisions + Uncontrolled Issues (or Neutral with Issues)
    elif quality == "needs_attention":
        summary = f"The discussion surfaced unresolved concerns regarding {topic} without clear decisions or ownership, indicating potential risk."  # noqa: E501

    # Case D (NEUTRAL): No Decisions + No Issues (Status Check)
    else:
        summary = f"The team discussed {topic}. Future actions need to be defined to move forward."  # noqa: E501
...
def compute_project_risk(issues_detected, risks_detected, uncontrolled_dependencies):
    ...
    return {"label": label, "score": risk_score, "drivers": drivers}


# --- OLD QUALITY ENGINE (DEPRECATED - KEPT FOR REFERENCE) ---


def compute_meeting_quality(
    ...
    if (
        signals.get("topic") != "general discussion"
        and signals.get("risk") != "elevated"
    ):
        score += 2
...
def assess_sales_signals(segments, objections, recommendations):
    ...
    has_hard_authority_risk = any(
        term in text_blob for term in HARD_AUTHORITY_RISK_TERMS
    )

    # Check if next step and timeline exist
    next_step_exists = len(recommendations) > 0 or any(
        t in text_blob for t in ["schedule next", "book a demo", "send the invite"]
    )
    timeline_exists = any(
        keyword in text_blob for keyword in COMMITMENT_KEYWORDS[:5]
    )  # Day-specific keywords

    # Only flag decision_authority_risk if hard risk AND no next step AND no timeline
    decision_authority_risk = (
        has_hard_authority_risk and not next_step_exists and not timeline_exists
    )
...
def compute_sales_quality(signals):
    ...
    if signals.get("end_of_call_commitment") or signals.get("hard_commitment"):
        ...
        if (
            "Strong end-of-call commitment" in drivers
            or "Hard commitment locked" in drivers
        ):
...
def detect_explicit_no_blockers(segments):
    ...
    for seg in tail:
        ...
        # which is hard with just looking at single segments, but let's try strict
        # keywords for now.
        if "no blocker" in text or ("none" in text and "blocker" in text):
            # Simplistic: "none for me re: blockers"
            return True
        # Check for specific "Any blockers?" -> "None" question/answer pattern requires
        # context,
...
def calculate_business_sentiment(segments, meeting_health):
    ...
    for s in segments:
        ...
        if "sentiment_score" not in s:
            ...
            raw_score = (
                0.5 if label == "Positive" else -0.5 if label == "Negative" else 0
            )
...
def evaluate_meeting_health(
    decisions, action_items, tension_points, sentiment_counts, segments
):
    """
...
INSIGHT_TEMPLATES = {
    ...
    "Execution Risk": "Potential execution risk identified in {topic} due to unclear next steps.",  # noqa: E501
    "Decision Ambiguity": "Key issues around {topic} were discussed, but no final decision was reached.",  # noqa: E501
    "Ownership Gap": "Follow-up actions related to {topic} lack clear ownership.",
    "Positive Momentum": "Clear progress made on {topic} with assigned actions and decisions.",  # noqa: E501
}
    ...
    if q == "High":
        ...
        if execution_attempted:
            insights.append(
                {
                    "type": "Decision Ambiguity",
                    "text": "Some execution elements remain unclear and require follow-up.",  # noqa: E501
                }
            )
        else:
            ...
            insights.append(
                {
                    ...
                    "text": "Direction and strategy were discussed. Detailed execution planning deferred to next meeting.",  # noqa: E501
                }
            )
...
def analyze_meeting(nlp_input: dict) -> dict:
    ...
    with profile_stage(session_id, "Decision extraction"):
        ...
        signals["decision_state"] = (
            "decision made" if signals["decision"] else "no final decision"
        )
    ...
    with profile_stage(session_id, "Action Item extraction"):
        ...
        signals["action_clarity"] = (
            "next steps identified" if action_items else "no clear next steps"
        )
    ...
    with profile_stage(session_id, "Conversation metrics calculation"):
        ...
        meeting_health = evaluate_meeting_health(
            decisions, action_items, actual_blockers, sentiment_counts, segments
        )
        ...
        biz_sentiment_score, biz_sentiment_label = calculate_business_sentiment(
            segments, meeting_health
        )
        ...
        # logger.info(f"🔍 QUALITY DEBUG: signals={signals},
        # meeting_quality={meeting_quality}")
        # logger.info(f"🔍 GUARANTEE: ownership={signals.get('ownership')},
        # execution={signals.get('execution_decision')}")
        # logger.info(f"🔍 RESULT: If ownership OR execution is True → Quality MUST be
        # Medium or High")

        # --- STEP 7: PROJECT RISK SCORING ---
        project_risk = compute_project_risk(
            signals["issues"], signals["risk"], uncontrolled_deps
        )
    ...
    return {
        ...
        "executive_signals": {
            **signals,
            "risk": "elevated" if (signals["issues"] or signals["risk"]) else "low",
        },  # Backwards compat
...
def detect_objections(segments, budget_alignment=False):
    ...
    for seg in segments:
        ...
        for clause in seg.get("clauses", []):
            ...
            objection_items = (
                OBJECTION_KEYWORDS.items()
                if isinstance(OBJECTION_KEYWORDS, dict)
                else [("General", OBJECTION_KEYWORDS)]
            )
...
def recommend_actions(objections, signals=None, segments=None):
    ...
    if call_stage in ["ambiguous", "weak"]:
        actions = [
            a
            for a in actions
            if "onboarding" not in a.lower() and "prepare" not in a.lower()
        ]
...
def compose_sales_summary(signals, quality, objections=None):
    ...
    """
    # Compose sales summary based on quality and objections

    # Template 4: Disqualification Override
    if quality["label"] == "Low":
        if signals.get("no_intent"):
            return "The prospect indicated no plans to switch or lack of interest. The opportunity is currently not active."  # noqa: E501
        if signals.get("deferred"):
            return "The prospect signaled a deferral to a later timeline. The opportunity is currently paused."  # noqa: E501
        ...
        if objections and len(objections) > 0:
            ...
            if len(objection_types) > 1:
                return f"Multiple concerns raised ({', '.join(objection_types).lower()}) without clear resolution. Opportunity requires re-qualification."  # noqa: E501
            else:
                return f"Concerns raised regarding {objection_types[0].lower()} without clear path forward. Follow-up needed to assess viability."  # noqa: E501

        # Fallback Low - Template 3: Early Exploration
        return "Early exploration phase. Buyer is gathering information. Follow-up needed to assess fit and timeline."  # noqa: E501
    ...
    if signals.get("end_of_call_commitment") or signals.get("hard_commitment"):
        ...
        if has_proposal and has_timeline:
            return "Strong progress made with proposal agreed and review scheduled. Clear buying signals detected with confirmed next steps."  # noqa: E501
        elif has_proposal or has_timeline:
            return "Strong progress made with clear commitment to next steps. The opportunity is advancing with confirmed actions."  # noqa: E501
        else:
            return "Strong progress made with hard commitment to next steps. The opportunity is advancing."  # noqa: E501
    ...
    if quality["label"] == "Medium":
        if signals.get("value_articulated") and signals.get("next_step"):
            return "Moderate progress made with buyer interest confirmed. Next steps identified but require firmer commitment and timeline."  # noqa: E501
        else:
            # Conservative for moderate without clear signals
            return "Discovery call completed with some interest shown. Requires follow-up to establish clear next steps and timeline."  # noqa: E501

    # Fallback - Conservative
    return "The discussion concluded without clear next steps or momentum. Additional qualification needed."  # noqa: E501
...
def generate_sales_insights(objections, signals, quality):
    ...
    if signals.get("no_intent") or signals.get("deferred"):
        insights.append(
            {
                "type": "Execution Risk",
                "text": "Prospect indicated no immediate intent or deferred timeline. Opportunity may require re-qualification.",  # noqa: E501
            }
        )
    ...
    if signals.get("hard_commitment") or signals.get("end_of_call_commitment"):
        insights.append(
            {
                "type": "Positive Momentum",
                "text": "Hard commitment secured with confirmed next steps. Strong buying signal detected.",  # noqa: E501
            }
        )
    ...
    if objections:
        ...
        if len(objection_types) > 2:
            insights.append(
                {
                    "type": "Execution Risk",
                    "text": f"Multiple objection types raised ({', '.join(objection_types)}). Requires comprehensive follow-up strategy.",  # noqa: E501
                }
            )
            ...
            insights.append(
                {
                    "type": "Positive Momentum",
                    "text": f"Objections around {', '.join(objection_types)} were addressed during the call.",  # noqa: E501
                }
            )
        else:
            insights.append(
                {
                    "type": "Decision Ambiguity",
                    "text": f"Unresolved concerns regarding {', '.join(objection_types)} require follow-up.",  # noqa: E501
                }
            )
    ...
    if signals.get("next_step") and not signals.get("hard_commitment"):
        insights.append(
            {
                "type": "Decision Ambiguity",
                "text": "Soft next step agreed but lacks firm commitment. Requires confirmation.",  # noqa: E501
            }
        )
        ...
        insights.append(
            {
                "type": "Execution Risk",
                "text": "No clear next steps defined. Follow-up strategy needed to maintain momentum.",  # noqa: E501
            }
        )

    # STEP 3: Decision Maker - Distinguish between shared decision-making and authority
    # risk
    if signals.get("decision_authority_risk"):
        ...
        insights.append(
            {
                "type": "Ownership Gap",
                "text": "Decision authority unclear and no next steps defined. May need to engage decision maker.",  # noqa: E501
            }
        )
        ...
        insights.append(
            {
                "type": "Decision Ambiguity",
                "text": "Shared decision-making process identified. Stakeholder alignment may be needed.",  # noqa: E501
            }
        )
        ...
        insights.append(
            {
                "type": "Decision Ambiguity",
                "text": "Decision maker not clearly identified. May need to engage additional stakeholders.",  # noqa: E501
            }
        )
...
def analyze_sales(enriched_segments: list, session_id: Optional[str] = None) -> dict:
    ...
    with profile_stage(session_id, "Buying Signal detection"):
        ...
        # Add text_blob to signals for template selection (STEP 7) and action generation
        # (STEP 5)
        text_blob = " ".join([s["text"].lower() for s in segments])
    ...
    with profile_stage(session_id, "Objection detection"):
        objections = detect_objections(
            segments, budget_alignment=sales_signals.get("budget_alignment", False)
        )

    # RULE 6: Context-aware action generation with stage-based mapping
    with profile_stage(session_id, "Action Item extraction"):
        recommendations = recommend_actions(
            objections, signals=sales_signals, segments=segments
        )

    # Compute quality with new signals and key insights
    with profile_stage(session_id, "Conversation metrics calculation"):
        quality = compute_sales_quality(sales_signals)
        # STEPS 3 & 4: Generate insights with authority classification and value gap
        # removal
