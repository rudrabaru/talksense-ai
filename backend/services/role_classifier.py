...
SALES_INDICATORS = [
    "thanks for joining", "let me show you", "our platform", "our product", 
    "our solution", "demo", "pricing plans", "implementation", "onboarding", 
    "customers use", "we provide", "let me explain"
]

CUSTOMER_INDICATORS = [
    "budget", "pricing", "how much", "interested", "can it", "does it", 
    "we currently", "our team", "our process", "concern", "problem", "challenge"
]
...
async def classify_roles(session_id: str) -> dict[str, str]:
    ...
    """
    logger.info("role_classifier — session %s: starting v1 classification", session_id[:8])
    
    async with AsyncSessionLocal() as db:
        segments = await crud.get_all_transcript_segments(db, session_id)
        
    if not segments:
        logger.warning("role_classifier — session %s: no segments found", session_id[:8])
        return {}
    ...
    logger.info("role_classifier — session %s: v1 classified roles -> %s", session_id[:8], roles)
    return roles


# ── V2 LLM Classifier (Gemini Flash) ─────────────────────────────────────────

ROLE_CLASSIFICATION_PROMPT = """You are an expert conversation analyst for a sales intelligence platform called TalkSense AI.

You are given a transcript of a conversation between two speakers. Your task is to determine which speaker is the **Sales Representative** and which is the **Customer**.
...
4. If the conversation is ambiguous or does not clearly fit a sales context, make your best judgment based on who is presenting/offering vs who is evaluating/asking.
...
async def classify_roles_v2(session_id: str) -> dict | None:
    ...
    """
    logger.info("role_classifier — session %s: starting v2 (Gemini) classification", session_id[:8])
    ...
    try:
        ...
        if not settings.gemini_api_key:
            logger.warning("role_classifier — session %s: GEMINI_API_KEY not configured, skipping v2", session_id[:8])
            return None
        ...
        if not segments:
            logger.warning("role_classifier — session %s: no segments for v2", session_id[:8])
            return None
        ...
        if len(speaker_texts) < 2:
            logger.warning("role_classifier — session %s: fewer than 2 speakers for v2", session_id[:8])
            return None
        ...
        prompt = ROLE_CLASSIFICATION_PROMPT.format(speaker_transcripts=speaker_transcripts)
        ...
        logger.info("role_classifier — session %s: Gemini raw response: %s", session_id[:8], raw_text[:200])
        ...
        if "roles" not in result:
            logger.error("role_classifier — session %s: v2 response missing 'roles' key", session_id[:8])
            return None
        ...
        logger.info(
            ...
            session_id[:8], result["roles"], result["confidence"]
        )
        return result
        
    except json.JSONDecodeError as e:
        logger.error("role_classifier — session %s: v2 JSON parse error: %s", session_id[:8], e)
        return None
    except Exception as e:
        logger.error("role_classifier — session %s: v2 Gemini call failed: %s", session_id[:8], e)
        return None
