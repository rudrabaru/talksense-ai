...
class NLPEngine:
    ...
    def __init__(self):
        ...
        try:
            ...
            logger.error(
                f"NLP Engine: Failed to load sentiment model (Offline?). Error: {e}"
            )
    ...
    def merge_semantic_segments(self, segments: list) -> list:
        ...
        for seg in segments:
            ...
            if buffer is None:
                buffer = (
                    seg.copy()
                )  # Copy to avoid mutating original list structure if needed
                continue
            ...
            # Rule 1 OR Rule 2
            should_merge = is_continuation_word or (
                not prev_ends_punct and curr_starts_lower
            )
        ...
        if self.sentiment_pipeline and texts_to_analyze:
            try:
                ...
                results = self.sentiment_pipeline(
                    texts_to_analyze, batch_size=len(texts_to_analyze)
                )
...
def get_nlp_engine() -> "NLPEngine":
    ...
    if _nlp_engine_instance is None:
        ...
        logger.debug(
            "NLP Engine: returning cached singleton (model resident in memory)."
        )
