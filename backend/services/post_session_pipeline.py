...
async def _generate_and_validate_with_retries(
    engine: LLMEngine, system_prompt: str, user_prompt: str, schema: dict
) -> dict:
...
async def run_post_session_pipeline(session_id: str) -> None:
    ...
    if not settings.enable_post_session_ai:
        logger.info(
            f"Post-Session AI is disabled via config. Skipping pipeline for {session_id[:8]}"
        )
        return
    ...
    async with AsyncSessionLocal() as db:
        try:
            ...
            db_segments = await crud.get_transcript_segments(
                db, session_id, limit=100000
            )
            if not db_segments:
                logger.info(
            ...
            if len(transcript_string) > MAX_TRANSCRIPT_CHARS:
                logger.warning(
                    f"Session {session_id[:8]} transcript exceeded {MAX_TRANSCRIPT_CHARS} chars. Truncating."
                )
            ...
            try:
                ...
                parsed_json = await asyncio.wait_for(
                    _generate_and_validate_with_retries(
                        engine, system_prompt, final_user_prompt, schema
                    ),
                    timeout=30.0,
                )
                ...
                logger.error(
                    f"Post-Session AI pipeline failed completely after retries for {session_id[:8]}: {str(e)}"
                )
            ...
            logger.info(
                f"[METRIC] Gemini Post-Session Latency: {gemini_latency:.2f}ms | Fallback: {fallback_used}"
            )
            ...
            if isinstance(raw_speaker_map, list):
                parsed_json["speaker_map"] = {
                    item.get("segment_id"): item.get("speaker")
                    for item in raw_speaker_map
                    if "segment_id" in item
                }
            if isinstance(raw_roles, list):
                parsed_json["roles"] = {
                    item.get("speaker"): item.get("role")
