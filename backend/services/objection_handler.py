...
async def analyze_objection_handling(db, session_id: str) -> None:
    ...
    try:
        ...
        objections_metric = next((m for m in metrics if m.metric_name == "objections"), None)
        roles_metric = next((m for m in metrics if m.metric_name == "speaker_roles"), None)
        buying_signals_metric = next((m for m in metrics if m.metric_name == "buying_signals"), None)
        
        if not objections_metric or not objections_metric.metric_value:
            return
            
        roles = roles_metric.metric_value if roles_metric and roles_metric.metric_value else {}
        sales_rep_speaker = next((k for k, v in roles.items() if v == "sales_rep"), None)
        customer_speaker = next((k for k, v in roles.items() if v == "customer"), None)
        
        if not sales_rep_speaker:
            logger.warning("objection_handler: No sales rep identified for session %s", session_id[:8])
            return
            
        objections = objections_metric.metric_value
        objection_handling_results = []
        
        buying_signals = buying_signals_metric.metric_value if buying_signals_metric and buying_signals_metric.metric_value else []
        
        for obj in objections:
            obj_text = obj.get("text", obj) if isinstance(obj, dict) else obj
            category = obj.get("category", "Pricing") if isinstance(obj, dict) else "Pricing"
            ...
            for s in segments:
                if s.start_time >= obj_segment.end_time and s.speaker_id == sales_rep_speaker:
                    response_segment = s
                    break
            ...
            if delay > 30.0:
                ...
                for bs in buying_signals:
                    ...
                    bs_seg = next((s for s in segments if bs_text in s.text and s.speaker_id == customer_speaker), None)
                    if bs_seg and bs_seg.start_time > response_segment.end_time and (bs_seg.start_time - response_segment.end_time) <= 60.0:
                ...
                if has_subsequent_buying_signal:
                    ...
                    if len(words) > 10 and (keyword in resp_text.lower() or "price" in resp_text.lower() or "plan" in resp_text.lower() or "offer" in resp_text.lower() or "value" in resp_text.lower() or "cost" in resp_text.lower() or "timeline" in resp_text.lower()):
        ...
        if objection_handling_results:
            ...
            logger.info("objection_handler: processed %d objections for %s", len(objection_handling_results), session_id[:8])
