...
def _apply_sandwich(segments):
    ...
    for i in range(1, len(segments) - 1):
        ...
        if prev_spk == next_spk and curr_spk != prev_spk:
            ...
            word_count = (
                len(segments[i].text.strip().split())
                if hasattr(segments[i], "text") and segments[i].text
                else 999
            )
...
def _apply_short_repair(segments):
    ...
    for i in range(1, len(segments) - 1):
        ...
        if prev_spk == next_spk and curr_spk != prev_spk:
            ...
            if (
                duration <= SHORT_REPAIR_MAX_DURATION_S
                and word_count <= SHORT_REPAIR_MAX_WORDS
            ):
                segments[i] = _clone_with_speaker(segments[i], prev_spk)
