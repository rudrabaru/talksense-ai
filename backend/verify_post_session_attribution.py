...
async def run_verification():
    ...
    stream_duration = (
        30.0  # Stream 30 seconds of audio to get multiple speaker segments
    )
    ...
    for step in range(30):
        async with AsyncSessionLocal() as db:
            result = await db.execute(
                text(
                    "SELECT status, speaker_attribution_status, audio_file_path FROM sessions WHERE id = :sid"  # noqa: E501
                ),
                {"sid": session_id},
            )
            row = result.fetchone()
            if row:
                status_log.append(
                    (
                        time.time(),
                        row.status,
                        row.speaker_attribution_status,
                        row.audio_file_path,
                    )
                )
                print(
                    f"    Time +{step}s | Session Status: {row.status:12s} | Attribution Status: {str(row.speaker_attribution_status)}"  # noqa: E501
                )
    ...
    if final_wav_path and os.path.exists(final_wav_path):
        ...
        try:
            with wave.open(final_wav_path, "rb") as w:
                ...
                print(
                    f"    WAV Properties: Channels={nchannels}, SampleWidth={sampwidth} bytes, FrameRate={framerate}Hz"  # noqa: E501
                )
    ...
    if "completed" in unique_transitions:
        ...
        print(
            "    ❌ TEST 2 FAILED: Attribution status did not transition to 'completed'."  # noqa: E501
        )
    ...
    async with AsyncSessionLocal() as db:
        result = await db.execute(
            text(
                "SELECT speaker_id, text, start_time, end_time FROM transcript_segments WHERE session_id = :sid ORDER BY start_time"  # noqa: E501
            ),
            {"sid": session_id},
        )
        ...
        print(
            f"    Distinct speakers found after post-session diarization: {distinct_speakers}"  # noqa: E501
        )
        if len(distinct_speakers) >= 2:
            print(
                f"    ✅ TEST 4 PASSED: DB updated with {len(distinct_speakers)} distinct speakers."  # noqa: E501
            )
        else:
    ...
    async with httpx.AsyncClient() as client:
        ...
        if resp.status_code == 200:
            ...
            if len(dash_speakers) >= 2:
                ...
                print(
                    "    ❌ TEST 5 FAILED: Dashboard does not return multiple speakers."
                )
        else:
            print(
                f"    ❌ TEST 5 FAILED: GET /dashboard status code {resp.status_code}"
            )
    ...
    async with AsyncSessionLocal() as db:
        ...
        if row is None:
            print(
                "    ❌ TEST 6 FAILED: No 'speaker_attribution' metric row found in session_metrics."  # noqa: E501
            )
        else:
            ...
            if missing:
                ...
                if diag["coverage_percent"] == expected_coverage:
                    print(
                        f"    ✅ TEST 6 PASSED: Attribution diagnostics persisted correctly (coverage={diag['coverage_percent']}%)."  # noqa: E501
                    )
                else:
                    print(
                        f"    ❌ TEST 6 FAILED: coverage_percent mismatch (expected {expected_coverage}, got {diag['coverage_percent']})."  # noqa: E501
                    )
    ...
    async with httpx.AsyncClient() as client:
        ...
        if resp.status_code == 200:
            ...
            if sa is None:
                print(
                    "    ❌ TEST 7 FAILED: 'speaker_attribution' key missing from dashboard response."  # noqa: E501
                )
            else:
                print("    speaker_attribution payload:")
                for k, v in sa.items():
                    print(f"      {k:<20} : {v}")
                ...
                if missing:
                    print(
                        f"    ❌ TEST 7 FAILED: Missing keys in speaker_attribution: {missing}"  # noqa: E501
                    )
                elif sa.get("status") != "completed":
                    print(
                        f"    ❌ TEST 7 FAILED: Expected status='completed', got {sa.get('status')!r}"  # noqa: E501
                    )
                elif sa.get("coverage") is None:
                    print(
                        "    ❌ TEST 7 FAILED: 'coverage' is None — diagnostics not populated."  # noqa: E501
                    )
                else:
                    print(
                        f"    ✅ TEST 7 PASSED: Dashboard speaker_attribution has all fields (coverage={sa['coverage']}%)."  # noqa: E501
                    )
        else:
            print(
                f"    ❌ TEST 7 FAILED: GET /dashboard status code {resp.status_code}"
            )
