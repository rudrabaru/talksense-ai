...
async def test_e2e():
    ...
    ffmpeg_cmd = ["ffmpeg", "-y", "-i", audio_path, "-f", "s16le", "-ac", "1", "-ar", "16000", "-"]
    ...
    for _ in range(30):
        async with AsyncSessionLocal() as db:
            res = await db.execute(text("SELECT status, speaker_attribution_status FROM sessions WHERE id = :sid"), {"sid": session_id})
            row = res.fetchone()
