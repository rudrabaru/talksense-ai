...
async def run_latency_test():
            ...
            while True:
                try:
                    ...
                    if data.get("type") == "transcript":
                        ...
                        recv_time = time.time()
                        # The audio corresponding to the segment ended at session_start_time + end_sec
                        audio_end_time = session_start_time + end_sec
                        latency = recv_time - audio_end_time
                        latencies.append(latency)
                        print(f"[Transcript Received] Speaker: {speaker} | Sentiment: {sentiment} | Text: '{text}'")
                        print(f"  -> Segment End: {end_sec:.2f}s | Latency: {latency:.3f}s")
    ...
    ffmpeg_cmd = [
        "ffmpeg", "-y", "-i", "../sample_audio/meeting_short.mp3",
