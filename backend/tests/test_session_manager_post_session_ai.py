...
def mock_db_crud(monkeypatch):
    monkeypatch.setattr("db.database.AsyncSessionLocal", MagicMock(return_value=AsyncMock()), raising=False)
    monkeypatch.setattr("ws.session_manager._flush_session_final", AsyncMock(), raising=False)
    monkeypatch.setattr("ws.session_manager._register_flush_worker", MagicMock(), raising=False)
...
def mock_pipeline(monkeypatch):
    mock_run = AsyncMock()
    monkeypatch.setattr("services.post_session_pipeline.run_post_session_pipeline", mock_run, raising=False)
...
async def test_post_session_ai_enabled(
    ...
    monkeypatch.setattr("ws.session_manager._run_post_session_diarization", AsyncMock(), raising=False)
    ...
    with patch("ws.session_manager._register_flush_worker") as mock_register:
        ...
        assert pipeline_scheduled, "Pipeline was not scheduled when feature flag is enabled"
