...
def mock_ml_models():
    with (
        patch("audio.transcriber.get_transcriber") as mock_t,
        ...
        mock_llm_instance.generate_structured.return_value = (
            '{"executive_summary": "Test", "decisions": [], "action_items": []}'
        )
...
def test_normal_successful_session(client: TestClient):
    ...
    r = client.post(
        "/sessions",
        json={"client_id": client_id, "mode": "meeting", "host_embedding": [0.0] * 512},
    )
    ...
    for _ in range(10):
        ...
        data = r.json()
        if data.get("post_session_ai") or data.get("speaker_attribution_status") in [
            "completed",
            "skipped",
            "failed",
        ]:
