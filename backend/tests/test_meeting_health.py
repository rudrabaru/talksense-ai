...
def test_meeting_health_scenarios():
    ...
    # Conditions: Decisions Made, Actions Assigned, No Blockers. Sentiment is Negative
    ...
    health_1 = evaluate_meeting_health(
        decisions_1, actions_1, blockers_1, sentiment_1, segments_1
    )
    print(f"Scenario 1 (Decisive, High Negativity): {health_1}")
    assert health_1 == "good"

    # Verify Key Insights suppression for Good outcome (e.g. Execution Risk should not
    # trigger even if actions are weird, though usually assigned actions implies strong)
    ...
    insights_1 = generate_key_insights(
        signals_1, actions_1, decisions_1, blockers_1, health_1
    )
    ...
    # Expect: Neutral (or Uncertain if sentiment was super negative, but here Positive
    # -> Neutral)
    decisions_2 = []
    actions_2 = []
    blockers_2 = []
    ...
    health_2 = evaluate_meeting_health(
        decisions_2, actions_2, blockers_2, sentiment_2, []
    )
    ...
    actions_5a = [
        {"task": "Get QA approval by tomorrow", "owner": "Bob", "timeline": "tomorrow"}
    ]
    # Note: Logic checks 'task' text for timestamp if 'timeline' key missing, or the
    # key.
