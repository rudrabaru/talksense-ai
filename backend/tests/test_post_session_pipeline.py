...
async def test_successful_execution(
    mock_settings, mock_db, mock_crud_get_transcript, mock_crud_save_analysis, mock_prompt_loader, monkeypatch
):
    monkeypatch.setattr("services.post_session_pipeline.AsyncSessionLocal", MagicMock(return_value=mock_db))
    
    mock_engine_cls = MagicMock()
    mock_engine_inst = MagicMock()
    mock_engine_inst.generate_json = AsyncMock(return_value='{"executive_summary": "success"}')
...
async def test_timeout_recovery_retry(
    ...
    mock_engine_inst.generate_json = AsyncMock(side_effect=[
...
async def test_authentication_failure_no_retry(
    ...
    mock_engine_inst.generate_json = AsyncMock(side_effect=AuthenticationError("bad key"))
    mock_engine_cls.return_value = mock_engine_inst
    monkeypatch.setattr("services.post_session_pipeline.LLMEngine", mock_engine_cls)
