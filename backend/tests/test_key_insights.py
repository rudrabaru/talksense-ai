...
def test_refined_key_insights():
    ...
    insights_guardrail = generate_key_insights(
        signals_risk_high, [], [], []
    )  # Empty tension
    print(
    ...
    # Scenario: Trigger Risk, Ambiguity, Gap (Unlikely due to collapse, but let's try to
    # force 3)
    # Risk, Gap, and ... maybe Positive? No, Positive needs decision.
    # Let's try: Risk (weak actions), Gap (all unassigned - wait, Gap requires actions.
    # If Risk (weak actions) usually implies Gap too if unassigned).
    ...
    actions_chaos = [
        {"task": "maybe fix", "owner": "Unassigned"}
    ]  # triggers Risk and Gap
    ...
    assert "Ownership Gap" not in [
        i["type"] for i in insights_capped
    ]  # Priority 3 (Dropped)
