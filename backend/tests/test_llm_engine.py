...
async def test_gemini_authentication_error(mock_settings):
    ...
    mock_model = MagicMock()
    mock_model.generate_content_async = AsyncMock(side_effect=PermissionDenied("Bad key"))
...
async def test_gemini_rate_limit(mock_settings):
    ...
    mock_model = MagicMock()
    mock_model.generate_content_async = AsyncMock(side_effect=ResourceExhausted("Quota exceeded"))
...
async def test_gemini_service_unavailable(mock_settings):
    ...
    mock_model = MagicMock()
    mock_model.generate_content_async = AsyncMock(side_effect=ServiceUnavailable("503 Backend Error"))
