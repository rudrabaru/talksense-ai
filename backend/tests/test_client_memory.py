...
from db.models import Client as DBClient, ClientSnapshot as DBClientSnapshot, Session as DBSession, SessionMetric as DBSessionMetric
...
async def test_client_memory():
    ...
    async with AsyncSessionLocal() as db:
        client = DBClient(id=client_id, name="Test Memory Enterprise", industry="Financial Technology")
        db.add(client)
        await db.commit()

    print("Asserting GET /clients/{client_id} returns default values when no snapshot exists...")
    ...
    # Add 1st completed session: sentiment=0.5, objections=["Pricing", "Pricing", "Integration"]
    s1_id = uuid.uuid4()
    async with AsyncSessionLocal() as db:
        s1 = DBSession(
            id=s1_id,
    ...
    assert "Financial Technology" in data["summary"] or "Test Memory Enterprise" in data["summary"]
    ...
    async with AsyncSessionLocal() as db:
        ...
        m2_objections = DBSessionMetric(
            ...
            metric_name="objections",
            metric_value=[
                {"type": "Integration", "text": "Database sync concerns"}
            ]
        )
    ...
    assert data["sentiment_trend"] == "improving", f"Expected improving, got {data['sentiment_trend']}"
    assert set(data["common_objections"]) == {"Integration", "Pricing"}

    assert data["last_meeting_date"] == "2026-06-21T14:00:00+00:00"
    print("[OK] Second meeting sentiment trend and objection frequency aggregation validated")

    # Add 3rd completed session: sentiment=0.5 (declining), objections=["Security", "Security"]
    s3_id = uuid.uuid4()
    async with AsyncSessionLocal() as db:
        s3 = DBSession(
            id=s3_id,
    ...
    assert data["sentiment_trend"] == "declining", f"Expected declining, got {data['sentiment_trend']}"
