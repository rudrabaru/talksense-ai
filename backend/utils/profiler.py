...
class SessionProfiler:
    ...
    def __init__(self, session_id: str):
        ...
        try:
            ...
            self.enabled = True  # fallback to True if config loader fails during bootstrap
...
@contextmanager
def profile_stage(session_id: Optional[str], stage_name: str) -> Generator[None, None, None]:
    """
    Context manager to profile a stage for a session.
    If session_id is None, unregistered, or profiling is disabled, operates with negligible overhead.
    """
    if not session_id:
        yield
        return
