"""
TalkSense AI — Pipeline Parameter Sweep Tool

Simulates the real-time streaming pipeline (250ms chunks, VAD, AudioBuffer, Whisper, Pyannote)
across the benchmark dataset to evaluate different:
1. Buffer Target Durations (1600ms, 1800ms, 2000ms)
2. Silero VAD Thresholds (0.55, 0.58, 0.60, 0.65)
"""
import os
import sys
import json
...
CATEGORIES = ["2_speaker", "3_speaker", "noisy"] # Use standard test categories for faster sweep
...
def run_simulation(audio_path, expected_speakers, transcriber, diarizer, vad):
    ...
    buffer = AudioBuffer()
    buffer._session_id = "sweep_sim"
    
    # 250ms chunk (8000 bytes for 16kHz 16-bit mono PCM)
    chunk_size = 8000 
    offset = 0
    all_diarized = []
...
def main():
    print("=" * 70)
    print("  TALKSENSE AI — PIPELINE PARAMETER SWEEP")
    print("=" * 70)
    ...
    print(f"Found {len(samples)} sweep samples.")
    
    # Define Sweep Axes
    durations = [1600, 1800, 2000]
    vad_thresholds = [0.55, 0.58, 0.60, 0.65]
    ...
    for target_dur in durations:
        for vad_thresh in vad_thresholds:
            print(f"\n--- Running Sweep: TARGET_DURATION_MS={target_dur}ms, VAD_THRESHOLD={vad_thresh} ---")
            
            # Configure constants dynamically
            audio_buffer.TARGET_DURATION_MS = target_dur
            audio_buffer.MIN_FLUSH_MS = min(800, target_dur // 2) # dynamic adjustment
            ...
            for sample_dir in samples:
                ...
                try:
                    predicted = run_simulation(audio_path, meta["expected_speakers"], transcriber, diarizer, vad)
                ...
                # Evaluate
                pred_eval = [EvalSegment(start=s.start, end=s.end, speaker=s.speaker, text=s.text) for s in predicted]
            ...
            print(f"Sweep Result: F1={avg_f1:.4f} | Accuracy={avg_acc:.1f}% | SCDR={avg_scdr:.1f}% | Time={elapsed:.1f}s")
