...
async def evaluate_session(session_id: str, ground_truth: Dict[str, Any] | None) -> Dict[str, Any]:
    ...
    if ground_truth:
        ...
        speaker_acc = (min(speakers_detected, exp_speakers) / max(speakers_detected, exp_speakers) * 100) if exp_speakers and speakers_detected else 0.0
        change_acc = (min(changes, exp_changes) / max(changes, exp_changes) * 100) if exp_changes and changes else 0.0
...
async def run_batch_evaluation(session_ids: List[str], truth_data: Dict[str, Any]) -> List[Dict[str, Any]]:
...
def generate_markdown_report(results: List[Dict[str, Any]]) -> str:
    ...
    if with_truth:
        avg_spk_acc = sum(r.get("speaker_accuracy", 0.0) for r in with_truth) / len(with_truth)
        avg_chg_acc = sum(r.get("change_accuracy", 0.0) for r in with_truth) / len(with_truth)
    else:
        avg_spk_acc = None
        avg_chg_acc = None
    ...
    lines.append("\n## Recommendation")
    # Condition: Avg Coverage > 80%, and if ground truth is present, reasonably high change accuracy.
    ...
    if is_safe:
        ...
        lines.append("\nSpeaker attribution quality is sufficient to support role classification.")
    else:
        lines.append("> **Role Classification Not Yet Safe**")
        lines.append("\nSpeaker attribution requires further tuning before role classification can be reliable.")

    if failures:
        lines.append("\n## Failure Analysis")
        for f in failures:
            lines.append(f"- **Session `{f['session_id']}`**: Coverage {f['coverage']:.1f}% (< 80%)")
    ...
    for r in results:
        ...
        lines.append(f"- **Speaker Changes**: {r['speaker_changes']} (SCDR: {r['scdr']:.1f}%)")
        
        if r["ground_truth"]:
            gt = r["ground_truth"]
            lines.append("- **Ground Truth Comparison**:")
            lines.append(f"  - Expected Speakers: {len(gt.get('expected_speakers', []))} (Accuracy: {r.get('speaker_accuracy', 0):.1f}%)")
            lines.append(f"  - Expected Changes: {gt.get('expected_changes', 0)} (Accuracy: {r.get('change_accuracy', 0):.1f}%)")
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate speaker attribution for TalkSense sessions.")
    parser.add_argument("session_ids", nargs="+", help="UUID(s) of the session(s) to evaluate")
    parser.add_argument("--ground-truth", help="Path to JSON file containing ground truth data", default=None)
