...
def main():
    ...
    configs = {
        "Phase 1: Baseline": {},
        "Phase 1: Strict Pyannote Cluster Threshold (0.7)": {"PYANNOTE_CLUSTERING_THRESHOLD": "0.7"},
        "Phase 1: Loose Pyannote Cluster Threshold (0.5)": {"PYANNOTE_CLUSTERING_THRESHOLD": "0.5"},
    ...
    best_pyannote = {"PYANNOTE_CLUSTERING_THRESHOLD": "0.7", "PYANNOTE_MIN_DURATION_OFF": "0.5"}
        
    phase2_config = {"FILTER_SHORT_UTTERANCES_S": "0.5"}
    results["Phase 2: Short Utterance Filtering (0.5s)"] = run_benchmark(phase2_config, "Phase 2: Short Utterance Filtering")
    
    phase3_config = {"STRICT_VAD_PREFILTER": "1"}
    results["Phase 3: Strict VAD Prefilter"] = run_benchmark(phase3_config, "Phase 3: VAD Prefilter")
    
    combined_config = {}
    combined_config.update(best_pyannote)
    combined_config.update(phase2_config)
    combined_config.update(phase3_config)
    results["Phase 4: Combined Optimization"] = run_benchmark(combined_config, "Phase 4: Combined")
