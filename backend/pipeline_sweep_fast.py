...
across two representative benchmark samples to quickly evaluate parameter configurations.
"""
import os
import sys
import json
...
# Select two representative samples (clean business meeting and conversational sales audio)
SELECTED_SAMPLES = [
...
def main():
    ...
    for target_dur in durations:
        for vad_thresh in vad_thresholds:
            print(f"\n--- Running Sweep: TARGET_DURATION={target_dur}ms, VAD_THRESHOLD={vad_thresh} ---")
            sys.stdout.flush()
