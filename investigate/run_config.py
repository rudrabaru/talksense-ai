...
class Seg:
    ...
    def ov(self, o):
        return max(0.0, min(self.end, o.end) - max(self.start, o.start))
...
def load_gt():
    with open(GROUND_TRUTH_PATH, "r", encoding="utf-8") as f:
        d = json.load(f)
    return [
        Seg(s["start"], s["end"], s["speaker"], s.get("text", ""))
        for s in d["segments"]
    ]
...
def load_wav():
    ...
    if nch > 1:
        a = a.reshape(-1, nch).mean(1).astype(np.int16)
...
async def fetch_db_segs():
    ...
    segs = [
        Seg(r.start_time, r.end_time, r.speaker_id or "Unknown", r.text or "")
        for r in rows
    ]
...
def run_pyannote(pipeline, waveform, kwargs):
    ...
    for turn, _, spk in ann.itertracks(yield_label=True):
        ...
        if spk.startswith("SPEAKER_"):
            try:
                m = f"Speaker {int(spk.split('_')[-1]) + 1}"
            except Exception:
                pass
...
def assign_speakers(turns, db_segs):
    ...
    for pred in db_segs:
        ...
        if best_spk == "Unknown" and turns:
            nearest = min(
                turns,
                key=lambda t: min(abs(pred.start - t.end), abs(pred.end - t.start)),
            )
...
def resolve_map(pairs):
    ...
    for p, a in pairs:
        cooc[p.speaker][a.speaker] += 1
    mp = {}
    used = set()
    for pl in sorted(cooc, key=lambda label: sum(cooc[label].values()), reverse=True):
        best_c = None
        best_n = 0
        for c, n in cooc[pl].most_common():
            if c not in used and n > best_n:
                best_n = n
                best_c = c
        if best_c:
            mp[pl] = best_c
...
def compute_metrics(pairs, label_map, assigned, annotated):
    ...
    for spk in all_c:
        ...
        pc = t + fp[spk]
        rc = t + fn[spk]
        p = (t / pc) if pc > 0 else 0.0
        r = (t / rc) if rc > 0 else 0.0
    ...
    def get_changes(segs, use_map=False):
        ...
        for i in range(1, len(segs)):
            ...
            if p != q:
                ch.append((segs[i - 1].end + segs[i].start) / 2)
        return ch
    ...
    coverage = round(
        sum(1 for s in assigned if s.speaker not in ("Unknown", ""))
        / max(len(assigned), 1)
        * 100,
        1,
    )
...
async def run():
    ...
    config_map = {
        ...
        "C": {
            "name": "C) min=2,max=2",
            "kwargs": {"min_speakers": 2, "max_speakers": 2},
        },
    }
    ...
    if not d._loaded:
        print("ERROR: Pyannote not loaded")
        sys.exit(1)
    ...
    print(
        f"SCDR       : {metrics['scdr']}% ({metrics['scdr_det']}/{metrics['scdr_tot']})"
    )
    print("\nPer-speaker:")
