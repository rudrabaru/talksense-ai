...
def main():
    ...
    for prefix, category, sample_name, expected_spk, desc in ASSIGNMENTS:
        ...
        if sample_name == "meeting_short" and os.path.isfile(existing_gt_path):
            shutil.copy2(existing_gt_path, os.path.join(sample_dir, "ground_truth.json"))
            print(f"  {category}/{sample_name}: Used existing human-verified GT")
        else:
            # Build GT from draft
            gt, label_map = build_gt_from_draft(draft, expected_spk, desc, draft["recording"])
            with open(os.path.join(sample_dir, "ground_truth.json"), "w", encoding="utf-8") as f:
                json.dump(gt, f, indent=2, ensure_ascii=False)
            print(f"  {category}/{sample_name}: Generated GT ({len(gt['segments'])} segs, {gt['expected_speakers']} speakers)")
